...
public interface APIInterface {
    ...
    Call<ProductCategoryResponse> getProductCategory(@Field("salesman_id") String salesman_id);

    @FormUrlEncoded
    @POST(ServerConfig.GET_PRODUCT_LIST)
    Call<ProductListResponse> getProductList(@Field("salesman_id") String salesman_id,
                                             @Field("category_id") String category_id);

    @FormUrlEncoded
    @POST(ServerConfig.GET_PRODUCT_OPTION_LIST)
    Call<ProductOptionListResponse> getProductOptionList(@Field("salesman_id") String salesman_id,
                                                         @Field("product_id") String product_id);

    @FormUrlEncoded
    @POST(ServerConfig.GET_ORDER_DETAIL)
    Call<OrderDetailResponse> getOrderDetail(@Field("salesman_id") String salesman_id,
                                             @Field("order_id") String order_id);

    @FormUrlEncoded
    @POST(ServerConfig.GET_ORDER_LIST)
    Call<OrderListResponse> getOrderList(@Field("salesman_id") String salesman_id);

    @FormUrlEncoded
    @POST(ServerConfig.GET_PAYMENT_LIST)
    Call<PaymentListResponse> getPaymentList(@Field("salesman_id") String salesman_id);

    @FormUrlEncoded
    @POST(ServerConfig.GET_PAYMENT_DETAIL)
    Call<PaymentDetailResponse> getPaymentDetail(@Field("salesman_id") String salesman_id,
    ...
    Call<PercentageResponse> getTaxPercentage(@Field("salesman_id") String salesman_id);
    ...
    Call<DealerListResponse> getDealerList(@Field("salesman_id") String salesman_id);

    @FormUrlEncoded
    @POST(ServerConfig.GET_DEALER_LIST)
    Call<DealerListFragmentResponse> getDealerFragmentList(@Field("salesman_id") String salesman_id);

    @FormUrlEncoded
    @POST(ServerConfig.GET_DAILY_REPORT_LIST)
    Call<DailyReportListResponse> getDailyReportFragmentList(@Field("salesman_id") String salesman_id);

    @FormUrlEncoded
    @POST(ServerConfig.GET_DAILY_REPORT_DETAIL)
    Call<DailyReportDetailResponse> getDailyReportDetail(@Field("salesman_id") String salesman_id,
                                                         @Field("id_daily_report") String id_daily_report);

    @FormUrlEncoded
    @POST(ServerConfig.GET_DASHBOARD)
    Call<DashboardResponse> getDashboard(@Field("salesman_id") String salesman_id);
    ...
    @Multipart
    @POST(ServerConfig.UPDATE_DEALER)
    Call<DealerRegistrationResponse> updateDealer(@Part("id") RequestBody dealer_id,
                                                  @Part("firm_name") RequestBody firm_name,
                                                  @Part("dealer_qualification") RequestBody dealer_qualification,
                                                  @Part("responsible_person_name") RequestBody responsible_person_name,
                                                  @Part("residential_address") RequestBody residential_address,
    ...
    @FormUrlEncoded
    @POST(ServerConfig.GET_DEALER_DETAIL)
    Call<GetDealerDataResponse> getDealerDetail(@Field("dealer_id") String dealer_id);

    @Multipart
    @POST(ServerConfig.ADD_DAILY_REPORT)
    Call<AddUpdateDailyReportResponse> addDailyReport(@Part("sales_man_id") RequestBody sales_man_id,
                                                      @Part("start_km") RequestBody start_km,
                                                      @Part("end_km") RequestBody end_km,
                                                      @Part("total_km") RequestBody total_km,
                                                      @Part("date") RequestBody date,
                                                      @Part("dealer_name") RequestBody dealer_name,
                                                      @Part("dealer_mobile_no") RequestBody dealer_mobile_no,
                                                      @Part("dealer_email") RequestBody dealer_email,
                                                      @Part("delear_business_name") RequestBody dealer_business_name,
                                                      @Part("dealer_location") RequestBody dealer_location,
                                                      @Part("dealer_used_crop_product") RequestBody dealer_used_crop_product,
                                                      @Part("dealer_demo_product") RequestBody dealer_demo_product,
                                                      @Part("mobile_exp") RequestBody mobile_exp,
                                                      @Part("dinner_exp") RequestBody dinner_exp,
                                                      @Part("staying_charges") RequestBody staying_charges,
                                                      @Part("other_exp") RequestBody other_exp,
                                                      @Part MultipartBody.Part farmer_delear_selfy_pic,
                                                      @Part MultipartBody.Part start_km_pic,
                                                      @Part MultipartBody.Part end_km_pic);

    @Multipart
    @POST(ServerConfig.UPDATE_DAILY_REPORT)
    Call<AddUpdateDailyReportResponse> updateDailyReport(@Part("sales_man_id") RequestBody sales_man_id,
                                                         @Part("id_daily_report") RequestBody id_daily_report,
                                                         @Part("start_km") RequestBody start_km,
                                                         @Part("end_km") RequestBody end_km,
                                                         @Part("total_km") RequestBody total_km,
                                                         @Part("date") RequestBody date,
